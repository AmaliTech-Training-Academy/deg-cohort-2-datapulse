...
class TestResetPasswordSuccess:
    def test_returns_200(self, api_client, verified_user, password_reset_token):
        response = api_client.post(
            URL, reset_payload(verified_user, password_reset_token)
        )
    ...
    def test_old_password_no_longer_works(
        self, api_client, verified_user, password_reset_token
    ):
        api_client.post(URL, reset_payload(verified_user, password_reset_token))
        response = api_client.post(
    ...
    def test_can_login_with_new_password(
        ...
        api_client.post(URL, reset_payload(verified_user, password_reset_token))
        response = api_client.post(
            LOGIN_URL, {"email": verified_user.email, "password": NEW_PASSWORD}
        )
        assert response.status_code == 200

    def test_token_is_invalidated_after_use(
        self, api_client, verified_user, password_reset_token
    ):
        api_client.post(URL, reset_payload(verified_user, password_reset_token))
        # Try to use the same token again with a different password
        response = api_client.post(
            URL, reset_payload(verified_user, password_reset_token, "Another@789")
        )
...
class TestResetPasswordFailure:
    ...
    def test_missing_uid_returns_400(self, api_client, password_reset_token):
        assert (
            api_client.post(
                URL, {"token": password_reset_token, "password": NEW_PASSWORD}
            ).status_code
            == 400
        )
    ...
    def test_missing_token_returns_400(self, api_client, verified_user):
        assert (
            api_client.post(
                URL, {"uid": str(verified_user.pk), "password": NEW_PASSWORD}
            ).status_code
            == 400
        )
