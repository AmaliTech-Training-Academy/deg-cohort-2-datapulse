...
class TestFullAuthFlow:
    ...
    def test_full_password_reset_flow(self, api_client, verified_user, mailoutbox):
        ...
        reset = api_client.post(
            RESET_URL, {"uid": uid, "token": token, "password": NEW_PASSWORD}
        )
        assert reset.status_code == 200

        # 4. Old password no longer works
        old_login = api_client.post(
            LOGIN_URL, {"email": verified_user.email, "password": "Secure@123"}
        )
    ...
    def test_complete_journey(self, api_client, mailoutbox):
        ...
        # 6. Reset password
        api_client.post(
            RESET_URL, {"uid": uid, "token": reset_token, "password": NEW_PASSWORD}
        )

        # 7. Login with new password
        final_login = api_client.post(
            LOGIN_URL, {"email": email, "password": NEW_PASSWORD}
        )
